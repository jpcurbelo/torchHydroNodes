...
def plot_metric_histogram(df_period, metric, threshold_dict, graph_title, period, plots_folder):
    ...
    n_basins = df['basin'].nunique()
    if metric in threshold_dict:
        ax.set_title(f'{graph_title} (${metric.upper()} \leq {th_value}$: {n_below_threshold}/{n_basins} basins) | {period} period')
    else:
        ax.set_title(f'{graph_title} | {n_basins} basins | {period} period')
...
def _plot_histogram(ax, df, metric, threshold_dict, graph_title, period):
    ...
    if metric in threshold_dict:
        if 'cluster' in graph_title.lower():
            ...
            ax.set_title(f'{graph_title} (${metric.upper()} \leq {th_value}$: {n_below_threshold}/{n_basins} basins) | {period}')
    else:
        if 'cluster' in graph_title.lower():
            ax.set_title(f'{graph_title} | {period}')
        else:
